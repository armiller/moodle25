...
//This plug-in, if I could make it work, would create a search box for the course table. But, I haven't been able to make it work.
require_once($CFG->dirroot.'/blocks/configurable_reports/plugin.class.php');

class plugin_fcurrentcoursefield extends plugin_base{
	
	function init(){
		$this->form = true;
		$this->unique = true;
		$this->fullname = get_string('fcurrentcoursefield','block_configurable_reports');
	...
	function execute($finalelements,$data){
		...
		$filter_fcurrentcoursefield = optional_param('filter_fcurrentcoursefield_'.$data->field,0,PARAM_RAW);		
		if($filter_fcurrentcoursefield){
			// addslashes is done in clean param
			$filter = clean_param(base64_decode($filter_fcurrentcoursefield),PARAM_CLEAN);
	...
	function print_filter(&$mform, $data){
		...
		$mform->addElement('select', 'filter_fcurrentcoursefield_'.$data->field, get_string($data->field), $filteroptions);
